...
let ownerID = "538349275713634315";
...
client.on('message', (message) => {
    ...
    switch (command) {
        ...
        case "queue":
            ...
            message.channel.send(`(Quick) **__Queue:__**\n\n${queueMsg}`);
            break;
                ...
                if (s.startsWith("https://www.youtube.com/watch?v=")) {
                    try {
                        yt.getInfo(s, (err, data) => {
                            ...
                            msg.edit(`:heavy_plus_sign:(Quick)Added song to queue: **${songName}**`);
...
client.on('message', message => {
  ...
  if (message.content === 'qpremiumjoin1') {
	   if (message.author.id !== '411137717884289024') return message.reply(':no_entry: | This Command Only For Premium  :fire: !')
 if(!message.author.id === '513642775259119636') return;
...
client.on('message', message => {
  ...
  if (message.content === 'qpremiumjoin2') {
	   if (message.author.id !== '513642775259119636') return message.reply(':no_entry: | This Command Only For Premium  :fire: !')
 if(!message.author.id === '513642775259119636') return;
    if (message.member.voiceChannel) {
      message.member.voiceChannel.join()
        .then(connection => { 
...
client.login(NTQxNjk1NDM1OTU4NjQ4ODQ3.DzjMvA.avlvekrU762Jhhkgrr_RRWjZG0w);
